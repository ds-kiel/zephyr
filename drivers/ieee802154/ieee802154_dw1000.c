...
struct {
	uint8_t count;
	struct irq_log_entry logs[50];
} irq_logs = {
	.count = 0,
};

#define SAVE_UINT32_TO_LOG(sys_stat) do { irq_logs.logs[irq_logs.count].value = sys_stat; irq_logs.count++; } while(0)
#define OUTPUT_DEBUG_LOGS() do { for(int i = 0; i < irq_logs.count; i++) { printk("%08x ", irq_logs.logs[i].value); } printk("\n"); irq_logs.count = 0;} while(0)
#else
#define SAVE_UINT32_TO_LOG(sys_stat)
#define OUTPUT_DEBUG_LOGS()
#endif
...
void output_timing_logs() {
	for(int i = 0; i < timing_logs.count; i++) {
		printk("%s: %llu [us]\n", timing_logs.logs[i].label, timing_logs.logs[i].timestamp_ns / 1000);
	}
}
...
static inline void dwt_switch_buffers(const struct device *dev) {
	...
	// check whether HSRBP and ICRBP are the same
	/* uint32_t sys_stat = dwt_reg_read_u32(dev, DWT_SYS_STATUS_ID, 0); */
	/* if((sys_stat & DWT_SYS_STATUS_ICRBP) == (sys_stat & DWT_SYS_STATUS_HSRBP)) { */
...
static inline void dwt_irq_handle_rxto(const struct device *dev, uint32_t sys_stat)
{
	...
	if(sys_stat & DWT_SYS_STATUS_RXRFTO) {
		...
		/* Although the documentation says no soft reset is necessary, I found that after a
		   preamble timeout, every 1 in 100 frames or something fails to demodulate.
		   performing a soft reset after a preamble timeout fixes this */
		dwt_disable_txrx(dev);
		dwt_reset_rfrx(dev);
...
static uint32_t last_rx_err_stat = 0;
...
static void dwt_irq_work_handler(struct k_work *item)
{
	...
	// work around double buffering bug see ERRATA: IRQ-1
	/* pin_state = gpio_pin_get_dt(&hi_cfg->irq_gpio); */
	k_sem_take(&ctx->dev_lock, K_FOREVER);

	sys_stat = dwt_reg_read_u32(dev, DWT_SYS_STATUS_ID, 0);
	/* SAVE_UINT32_TO_LOG(sys_stat); */

	/* LOG_ERR("b 0x%08x", sys_stat); */
	if (sys_stat & DWT_SYS_STATUS_RXFCG) {
		if (atomic_test_bit(&ctx->state, DWT_STATE_IRQ_POLLING_EMU)) {
			// like in the flow diagram, only clear after reading data
	...
	if(sys_stat & DWT_SYS_STATUS_ALL_DBLBUFF) {
			      ...
			      struct gpio_callback *cb, uint32_t pins)
{
	...
	const struct dwt_hi_cfg *hi_cfg = ctx->dev->config;

	// only dispatch if gpio line is high
	pin_state = gpio_pin_get_dt(&hi_cfg->irq_gpio);
	if(pin_state > 0) {
...
static int dwt_initialise_dev(const struct device *dev)
{
	...
	if (!(dwt_reg_read_u8(dev, DWT_SYS_STATUS_ID, 0) &
		...
		dwt_reg_write_u32(dev, DWT_SYS_STATUS_ID, 0, DWT_SYS_STATUS_CPLOCK);
	}
...
#warning "EXPERIMENTATION WITH NTM value for random access outlier detection"
	/* dwt_reg_write_u8(dev, DWT_LDE_IF_ID, DWT_LDE_CFG1_OFFSET, ((3 << 5) | 30)); */
...
struct mtm_ranging_timing mtm_ranging_conf = {
	.phy_activate_rx_delay = US_TO_DWT_TS(128 + 36), //+36 // give receiver some more time to start up
	.phase_setup_delay = US_TO_DWT_TS(200),
	.round_setup_delay = US_TO_DWT_TS(200),
	.preamble_timeout = (128/8),
	/* .preamble_timeout = (64/8), */
	.preamble_chunk_duration = US_TO_DWT_TS(8), // 8 symbols per cross-correlated chunk
};
...
int deca_glossy_time_synchronization(const struct device *dev,
	...
	if (conf->isRoot) {
		k_sem_take(&ctx->dev_lock, K_FOREVER);
		/* uint64_t dwt_start_ts; */
		initiator_rtc_ts = k_cycle_get_32() + 2;
		z_nrf_rtc_timer_set(1, initiator_rtc_ts, read_deca_system_timestamp, dev);
		if(k_sem_take(&read_dwt_sys_clock, K_MSEC(100)) != 0) {
			printk("Failed to read system timestamp\n");
			ret = -EIO;
			goto cleanup;
		}
		...
		initiator_dwt_ts = (dwt_start_ts + US_TO_DWT_TS(conf->transmission_delay_us + conf->guard_period_us)) % DWT_TS_MASK;
		...
		for(size_t k = 0; !success && (k < conf->max_depth || !conf->max_depth); k++) {
			...
			if(irq_state == DWT_IRQ_RX) {
				...
				if(buf[0] != DWT_MTM_GLOSSY_TX_ID) {
					dwt_switch_buffers(dev);
					k_sem_give(&ctx->dev_lock);
					printk("Wrong frame id received, expected %u, got %u\n", DWT_MTM_GLOSSY_TX_ID, buf[0]);
					ret = -EIO;
					goto cleanup;
				}
				...
				dwt_fast_enable_tx(dev, (local_dwt_ts + US_TO_DWT_TS(conf->transmission_delay_us)) % DWT_TS_MASK);
				...
				// TODO check whether we want to offset by the guard period, in theory theres no strict requirement to perfectly align mcu and DWT time (uint64_t) US_TO_DWT_TS(conf->guard_period_us);
				dwt_inst->local = local_dwt_ts - (glossy_frame.hop_count-1) * (uint64_t) US_TO_DWT_TS(conf->transmission_delay_us);
	...
	if(irq_state == DWT_IRQ_ERR) {
		ret = -EIO;
		printk("Error in Glossy transmission, SYS_STATUS: 0x%08x\n", dwt_reg_read_u32(dev, DWT_SYS_STATUS_ID, 0));
		goto cleanup;
	} else if (irq_state == DWT_IRQ_FRAME_WAIT_TIMEOUT ||
		   irq_state == DWT_IRQ_PREAMBLE_DETECT_TIMEOUT) {
		printk("Error in Glossy transmission, SYS_STATUS: 0x%08x\n", dwt_reg_read_u32(dev, DWT_SYS_STATUS_ID, 0));
		ret = -ETIMEDOUT;
		goto cleanup;
	...
	struct deca_ranging_digest *digest)
{
	...
	k_sem_take(&ctx->dev_lock, K_FOREVER);
	dwt_setup_rx_timeout(dev, 1200); // max packet transmission will take about 700us
	...
	for(size_t s = 0; s < schedule->slot_count + 1; s++) {
		...
		if(type == DENSE_LOAD_TX_BUFFER) {
			...
			for(size_t i = s; i < schedule->slot_count; i++) {
				...
				current_frame_transmission_ts += US_TO_DWT_TS(slot->duration_us);
			}
			...
			if(future_tx_slot != UINT16_MAX) {
				...
				current_frame_transmission_ts =
					((current_frame_transmission_ts + ranging_conf->phy_activate_rx_delay
						+ US_TO_DWT_TS(conf->guard_period_us)/2
						+ NS_TO_DWT_TS(conf->micro_slot_offset_ns)
...
// 4096 is maximum amount we will ever grab here, + 1 because there will always be one garbage byte
		...
		slot_start_ts = (slot_start_ts + US_TO_DWT_TS(current_slot->duration_us)) & DWT_TS_MASK;
	}
